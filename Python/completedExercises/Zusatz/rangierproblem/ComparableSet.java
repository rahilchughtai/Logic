               ...
               Iterable<T>
{
    ...
    public boolean isSubset(ComparableSet<T> set) {
     ...
     // end toList;
    private ComparableSet<ComparableList<T>> toList(ComparableSet<T> S)
    {
    	ComparableSet<ComparableList<T>> set =  new ComparableSet<ComparableList<T>>();
    	if(S.isEmpty())
    		set.add(new ComparableList<T>());
    	else
    	{
    		for(T x: S)
    		{
    			ComparableSet<T> xSet = new ComparableSet<T>();
    			xSet.add(x);
    			for(ComparableList<T> l: toList(S.difference(xSet)))
    			{
    				ComparableList<T> list = new ComparableList<T>();
        			list.add(x);
    				list.addAll(l);
    				set.add(list);
    			}
    ...
    public ComparableSet<ComparableList<T>> toList()
    {
    	return toList(this);
    }
    ...
    // Add the given element to all sets that are members of the set setOfSets.
    ...
    // Create a deep copy of the given set.  It is important that the
    // sets contained in the given set are copied themselves.
